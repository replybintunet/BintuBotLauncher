...
current_processes = []  # To support multiple streams
...
form {
    background: rgba(255,255,255,0.9); border-radius: 15px; box-shadow: 0 8px 20px rgba(0,0,0,0.2);
    padding: 25px 35px; max-width: 500px; width: 100%; animation: float 6s ease-in-out infinite;
}
...
def start():
    ...
    if cf_url:
        send_telegram(f"📺 Cloudflare Forwarding Link Active (hidden from UI)")
...
def stop_all():
    ...
    log("✅ All streams stopped by user.")
    return index()
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000)render_template_string(HTML, log=log_content, streams=list(streams.keys()))
